...
From our residual plots, we can see that the variances for each treatment combination of age group and gender are similar in covid 19 and pneumonia Our equal variance assumption is not violated. However for Influenza the variance for the first group is zero since all of the values are zero. This group is vastly different from the other groups so this violates the constant variance assumption.
...
We can see that the pneumonia interaction plot seems to have no interaction plot as the lines are closely parallel. We can also see that there is Factor A effect since the mean line is not horizontal. There is no Factor B effect since the means of the lines seem to be at the same level. We will test this further. 
...
Conclusion: Since the test statistic is less than the critical value, we fail to reject the null hypothesis at the 0.05 significance level and think that there is no interaction effect present in our study. 
...
Conclusion: Since the test statistic is greater than the critical value, we reject the null hypothesis at the 0.05 significance level and think that there is factor B effect (Sex) in our study.
...
Conclusion: Since the test statistic is greater than the critical value, we reject the null hypothesis at the 0.05 significance level and think that there is factor A effect (Age group) in our study.
...
Conclusion: Since the test statistic is less than the critical value, we fail to reject the null hypothesis at the 0.05 significance level and conclude that there is no factor B effect (Sex) in our study.
...
In the pneumonia interaction plot, we can see that there is interaction effect because the lines do intersect. We can also see that there is Factor A effect as the mean line is not horizontal. We can also see that there is Factor B effect as the means of the lines are not at the same level. We will test this further. 
...
Conclusion: Since the test statistic is greater than the critical value, we reject the null hypothesis at the 0.05 significance level and think that there is an interaction effect present in our study. 
...
Conclusion: Since the test statistic is greater than the critical value, we reject the null hypothesis at the 0.05 significance level and think that there is a factor A effect (Age group) in our study.
...
Most of the pneumonia cases probably resulted from Covid 19 cases that had escalated over time. Influenza had an abnormally small amount of deaths most likely because of public health guidelines such as mask mandates and social distancing. We can expand upon our analysis from this study by studying specific individuals who have the same demographics (age, gender,income) rather than the proportions of deaths caused by the diseases in various states.
